...
class NetworkErrors {
  static void handleNetworkErrors(http.Response response) {
    ...
    if (response.statusCode == 404) {
      Toasts.showToast(Colors.red, result['message'] ?? 'Account not found, please create an account');
    } else if (response.statusCode == 400) {
      Toasts.showToast(Colors.red, result['message'] ?? 'Please provide all fields');
